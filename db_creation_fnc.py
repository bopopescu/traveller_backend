...
class DB:
    ...
    def __init__(self, db="database", host="localhost", port=3306, user="root", password="password"):
        self.db = mysql.connector.connect(
            host=host,
            port=port,
            user=user,
    ...
    def select_from_table(self, table_name, columns="*", where_condition="", like_condition="", multiple=False):
        cursor = self.db.cursor(buffered=True, dictionary=True)
