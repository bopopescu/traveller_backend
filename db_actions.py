...
def updateProfile(user_id,name,surname,birthday,nationality,url_picture,languages_list="languages",interests_list="interests"):
    user_id = db.select_from_table("user",where_condition = "user_id = '" + user_id + "'")
    return user_id
...
def addChat(user_id1,user_id2):
    ...
    conv_id = db.insert_in_table("conversation",conv_info,columns = "(user_id1,user_id2)")
    return conv_id
...
def addMessage(conv_id, sender_user_id, text, url_resource=""):
    ...
    mes_id = db.insert_in_table("message",message,columns = "(conv_id,sender_user_id,text,url_resource)")
    return mes_id
